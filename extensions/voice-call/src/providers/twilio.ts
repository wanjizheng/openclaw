  ...
  async playTts(input: PlayTtsInput): Promise<void> {
    ...
    if (this.isHybridMode && input.audioUrl) {
      ...
      if (queue) {
        ...
        // Mark this single-chunk turn as done so the redirect handler resumes
        // CR after playback (Phase 2 will defer this until streaming finishes).
        queue.markDone(input.providerCallId, false);
        return;
      }
    }
